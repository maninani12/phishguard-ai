...
Selected model: **Decision Tree**. Accuracy 0.9799; phishing precision 0.9842; phishing recall 0.9685; phishing F1 0.9763; training score 0.9866; test score 0.9799; gap 0.0067; false positives 312; false negatives 630. Confusion matrix (actual rows, predicted columns; class order 0 phishing, 1 legitimate): `[[19379, 630], [312, 26658]]`. Domain-aware accuracy 0.9793, phishing precision 0.9831, phishing recall 0.9686, F1 0.9758, false positives 339, false negatives 641. See generated comparison and robustness reports for all model and URL subgroup metrics.
...
The source dataset contains webpage-level predictors that would violate the URL-only requirement; these are deliberately ignored. The URL-only audit finds a structural bias: legitimate/phishing scheme counts are `{0: {'http': 51502, 'https': 48543}, 1: {'http': 0, 'https': 134849}}`; www/non-www counts are `{0: {False: 58435, True: 41610}, 1: {False: 0, True: 134849}}`; `.com`/`.in` counts are `{0: {'com': 43453, 'in': 535}, 1: {'com': 68785, 'in': 842}}` (label 0 is phishing, label 1 legitimate). In the source, legitimate records are all HTTPS and all use `www.`; there are no labeled legitimate non-`www` records. The test split includes 6,599 non-`www` root URLs, all labeled phishing, so it cannot establish that unseen bare domains are legitimate. `.com` and `.in` occur in both classes. The selected model is chosen by measured held-out phishing F1, with accuracy and precision as tie-breakers; predictions remain statistical results and not ground-truth validation. Random URL splits can share hostnames, so hostname-disjoint metrics are reported separately. Predictions can be wrong, and dataset-level class patterns may not match current traffic.
